...
from app.tasks.analysis import run_workflow_task
...
async def post_usermessage(request: Request) -> Dict[str, Any]:
    ...
    # Quick Reply (Phase 1 placeholder)
    quick_reply = "分析を開始しました。しばらくお待ちください。"

    # Note: Currently we don't save the 'quick reply' to DB as the AI response,
    # because the actual response will be generated by the task.
    # Or should we? If we don't save it, the user sees it in UI but not in history?
    # For now, we return it. The task will save the final AI message.
